...
public class DualCoreProcessor {
    ...
    /*java program for There is a dual core processor with one queue for each core. There are n processes, where the time to complete the (i)th process is denoted by time[i] (0 <= i <= n-1). There is a latch that helps decide which process is completed by which core. Initially, the first core has the latch. Suppoes the latch is currently with the (c)th core, where c is either 1 or 2, and the (i)th process needs to be assigned. Then one of the following operations must be performed:
...
*/
    ...
    private static Result solve(int[] time,
        ...
        if (latch == 1) {
            ...
            // Option 1:
            // Give current process to Core 1
            Result r1 =
                    solve(time, i + 1, 2);
            ...
            // Core 1 maximizes own sum
            if (takeSelf.s1 >= giveOther.s1) {
                best = takeSelf;
            } else {
                best = giveOther;
            }
        ...
        // Latch with Core 2
            ...
            // Core 2 maximizes own sum
            if (takeSelf.s2 >= giveOther.s2) {
                best = takeSelf;
            } else {
                best = giveOther;
            }
